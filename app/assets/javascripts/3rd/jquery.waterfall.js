...
;(function($) {
	...
	$.extend(Waterfall.prototype, {
		options: {
			...
			updateDelay: 100, //how often to reflow layout on window resize
		...
		_create: function(opts) {
			...
			var cStyle = getStyleProperty(self.el);
			self.el.hidden = true;

			self.$el.css('minHeight', cStyle.height); //prevent scrollbar width changing
		...
		_initLayoutParams: function() {
			var self = this,
				o = self.options,
				cStyle = getStyleProperty(self.el),
				i = 0,
			...
			self.lastItems.length = o.colLength;
		...
		_setItemWidth: function(el) {
			...
			if (this.options.useCalc) {
				el.w = (100 * colWeight);
				el.style.width = this.prefixedCalc + '(' + (100 * colWeight) + 'vw - ' + (el.mr + el.ml + (this.pl + this.pr) * colWeight) + 'px)';
			} else {
	...
	$.fn.waterfall = function(arg, arg2) {
		if (typeof arg == 'string') { //Call API method
			...
			var $this = $(this),
				opts = $.extend({}, {"colLength": ~~$this[0].getAttribute("data-col-length")}, arg);
			if (opts.width && !opts.colLength) {
				opts.colLength = opts.width;
			}
