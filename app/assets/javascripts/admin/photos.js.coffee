  ...
  $('.js_admin').on 'click', '.js_admin_upload_field', ->
    ...
    .on 'fileuploadadd', (e, data) ->
      ...
      $.each data.files, (index, file) ->
        data.context.append($("<div/>").addClass('photo-uploader__file-name').text(file.name).prepend($("<i/>").addClass("black icon file")))
        ...
        upload_button = $("<a/>").addClass("ui teal label large").prop("disabled", true).text("上传").on "click", ->
          $this = $(this)
          button_data = $this.data()
          $this.off("click").removeClass('teal').addClass('black').text("取消").on "click", ->
            $this.remove()
            button_data.abort()
    ...
    .on 'fileuploaddone', (e, data) ->
      unless data.image_id
        ...
        data.context.append $('<div/>').addClass('photo-uploader__file-url').text(data.result.image.url).prepend($("<i/>").addClass("black icon url"))
