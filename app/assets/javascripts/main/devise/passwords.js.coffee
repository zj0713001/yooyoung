$ ->
  $('.js_main_passwords_send_sms_captcha').on 'click', ->
    user_phone = $('.js_main_passwords_phone_field').val()
    if $(this).hasClass('ajax_disabled')
      return false
    if !_.isEmpty($('.js_main_passwords_phone_field').val()) && $('.js_main_passwords_phone_field').valid()
      $.ajax
        url: $(this).data('url')
        type: 'GET'
        dataType: 'json'
        ...
        success: (data) ->
          unless data.status == true
            $('.js_main_password_sms_captcha_button').hide()
            $('.js_main_password_sms_captcha_sending').show()
            count_down()
          else
            ...
            $error_group.insertAfter $('.js_main_passwords_sms_captcha_field')
        complete: ->
          $(this).removeClass('ajax_disabled')
          $('.js_main_passwords_sms_captcha_field').prop('disabled', false).focus()
  count_down = ->
    $time_view = $('.js_main_password_sms_captcha_sending em')
    ...
    if time > 1
      ...
      $('.js_main_password_sms_captcha_sending').hide()
      $('.js_main_password_sms_captcha_button').show()
      $time_view.data('time', 60)
      $('.js_main_passwords_send_sms_captcha').removeClass('ajax_disabled')

  $('.js_main_passwords_new_form').validate
    rules:
      'user[phone]':
        required: true
        isMobile: true
      ...
      'user[password_confirmation]':
        required: true
        equalTo: '.js_main_passwords_password_field'
      'sms_captcha':
        required: true
        number: true
        minlength: 6
  ...
  $('.js_main_passwords_phone_field').on 'blur', ->
    if !_.isEmpty(user_phone = $('.js_main_passwords_phone_field').val()) && $('.js_main_passwords_phone_field').valid()
      $('.js_main_passwords_password_field').trigger 'focus'
      ...
      $.ajax
        ...
        success: (data) ->
          unless data.status == false && data.error_code == 6
            message = '电话号码尚未注册。'
  ...
  $('.js_main_passwords_new_form').on 'submit', ->
    return false if $(this).hasClass('ajax_disabled')
    $('.js_main_form_error_captcha').remove()
      ...
      $.ajax
        ...
        success: (data) ->
          if data.status == true
            ...
            message = if data.error_code == 10 then '验证码错误，请您核对，30分钟内输入有效。' else '密码找回出现问题，请联系网站客服处理。'
            $error_group = $('<div></div>').addClass('main-form__error-group-clearfix js_main_form_error_captcha').append($('<div></div>').addClass('main-form__error-group').append($('<div></div>').addClass('main-form__error-arrow')).append($('<div></div>').addClass('main-form__error-message').text(message)))
            $error_group.insertAfter $('.js_main_passwords_sms_captcha_field')
        complete: ->
          $(this).removeClass('ajax_disabled')
