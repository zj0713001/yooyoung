class NotificationService
  ...
  def send_registration_captcha(user, captcha)
    raise 'out of max nubmer' if Notification.where(notification_type: :send_registration_captcha, channel: :sms, identifier: user.phone, :created_at.gt =>  Time.new.midnight).count >= Settings.sms.sms_limit.to_i
